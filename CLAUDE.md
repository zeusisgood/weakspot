...
- バージョンの方針（1.1.0 以降）:
  ...
  - 現行は 1.8.9。範囲は `WeakSpotMod.ACCEPTED_VERSIONS = "[1.8,1.9)"`（Maven のバージョン範囲の書式。Forge の `VersionRange`）。
...
`doc/spec/SPEC_v1.9.0.md`（通信を変える整理・設定の追加。1.8.x と接続不可）。ユーザーの「実装」済み（1.8.9 をリリースしたあとに続けて行う）。
