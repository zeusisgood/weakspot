...
- バージョンの方針（1.1.0 以降）:
  ...
  - 現行は 1.3.0。範囲は `WeakSpotMod.ACCEPTED_VERSIONS = "[1.3,1.4)"`（Maven のバージョン範囲の書式。Forge の `VersionRange`）。
...
- `client/`（`@EventBusSubscriber(value = Side.CLIENT)`。専用サーバーではロードされない）: 弱点の状態、ヒット判定、描画、ヒット音、クライアント側のブースト。弱点は一度に1つ（`ClientWeakSpotHandler.spot`）。
  ...
  - 近接の弱点（1.3.0、`HitKind.MELEE`、`MeleeTargets`・`server/MeleeHits`）: 攻撃が届く距離で敵（`IMob` の生き物。プレイヤーを除く）に照準を合わせると、動物と同じく箱の面に出す（`WeakSpot.spawnOnEntity(HitKind.MELEE, …)`。持ち物は問わない。右クリック中・採掘中は出さない）。ヒットは `MouseEvent`（左クリックの押下。攻撃のキーの処理より先に来る）で、照準の点がその tick の箱の弱点に入っていて、攻撃のゲージが溜まっている（`getCooledAttackStrength(0.5) > 0.9`。溜まっていない間は濃さ 0.35 倍）ときだけ。バニラの攻撃は止めない。ヒット通知は攻撃のパケットより先にサーバーのスレッドの予定に積まれるので、`MeleeHits` がヒットを覚え（5 tick）、続く `AttackEntityEvent`（LOWEST。バニラは `CriticalHitEvent` の前に `resetCooldown` するので、ゲージはここで 2 tick 甘く確かめる）で対象を決め、`CriticalHitEvent` を ALLOW・倍率 1.5 にする。統計の `critHits` は実際にクリティカルにしたときに数える。他のプレイヤーには見せない。透かしは `animalSpotSeeThrough` に従う。
